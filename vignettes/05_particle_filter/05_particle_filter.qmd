...
This vignette implements a bootstrap particle filter for likelihood estimation, equivalent to dust2's `dust_filter_create()` / `dust_likelihood_run()`. We build a simple particle filter from scratch using the stochastic SIR model, then compute log-likelihoods and explore the likelihood surface.
