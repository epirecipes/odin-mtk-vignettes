---
title: "Incidence Tracking"
subtitle: "ModelingToolkit.jl Vignette 03"
format:
  html:
    toc: true
    code-fold: false
...
Tracking incidence (new cases per time period) is essential for fitting models to surveillance data. In R odin2, this is done with the `zero_every` feature. Here we implement the same pattern in Julia using a discrete-time simulator that resets the incidence counter at each observation time.
